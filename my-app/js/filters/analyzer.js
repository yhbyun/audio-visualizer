...
var bufferFactor = 1;
var frameBufferSize = 4096 * bufferFactor;
...
AnalyzerFilter.prototype.process = function(buffer) {
  ...
  /*
  var height= Math.random() * maxH;
  $('.bar').eq(0).css('height', height + 'px');
  $('#bar').eq(0).css('height', height + 'px');
  $('#status').text(height);
  return;
 */

  for (var i = 0, j = 0, n = buffer.length; i < n; i += 2, j++) {
    left = buffer[i];
    right = buffer[i + 1];
    signal[j] = (left + right) / 2;
  }

  logger.silly('buffer length=' + n);
  logger.silly('signal length=' + j);

  fft.forward(signal);
  var avgSpectrum = getAvgSpectrum();

  //console.log(avgSpectrum.length);
...
function getAvgSpectrum(){
  var spectrum = [],
  ray = 0;
  for(var i = 0, fftLen = (fft.spectrum.length/(16 * bufferFactor)); i < fftLen; i++ ){
    for(var j =0; j < (16 * bufferFactor); j++){
      ray += fft.spectrum[i* (16 * bufferFactor) + j];
    }
    ray /= (16 * bufferFactor);
    spectrum.push(ray);
  }
  logger.silly('spectrum length=' + fft.spectrum.length);
  return spectrum;
}
